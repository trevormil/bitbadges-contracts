...
use bitbadges_bindings::{SwapMsg, BitbadgesMsg};
...
pub fn mock_instance(
    ...
    // let bitbadges_features = features_from_csv("staking,bitbadges");
    // check_wasm(wasm, &bitbadges_features).unwrap();
...
fn proper_initialization() {
    ...
    let res: InitResponse<BitbadgesMsg> = init(&mut deps, env, msg).unwrap();
...
fn buy_limit() {
    ...
    let _res: InitResponse<BitbadgesMsg> = init(&mut deps, env, msg).unwrap();
    ...
    let res: HandleResponse<BitbadgesMsg> = handle(&mut deps, env, msg).unwrap();

    // make sure we produce proper trade order
    assert_eq!(1, res.messages.len());
    if let CosmosMsg::Custom(BitbadgesMsg::Swap(SwapMsg::Trade {
        trader_addr,
        offer_coin,
        ask_denom,
    })) = &res.messages[0]
